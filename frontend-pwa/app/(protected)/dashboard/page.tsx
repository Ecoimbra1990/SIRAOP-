...
export default function DashboardPage() {
  ...
  const handleGerarInformativo = async () => {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="flex items-end">
              <button
                onClick={handleGerarInformativo}
                disabled={selectedOcorrencias.length === 0 || isGeneratingPDF}
                className="w-full bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Download className="h-4 w-4" />
                {isGeneratingPDF ? 'Gerando...' : `Gerar PDF (${selectedOcorrencias.length})`}
              </button>
            </div>
                        ...
                        <button
                          onClick={() => {
                            if (confirm('Tem certeza que deseja excluir esta ocorrência?')) {
                              ocorrenciasApi.delete(ocorrencia.id).then(() => {
                                loadOcorrencias();
                              });
                            }
                          }}
